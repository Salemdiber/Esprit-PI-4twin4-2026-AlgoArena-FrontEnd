...
import { getOAuthUrl } from '../../services/oauthRoutes';
...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setErrorMsg('');
        if (!recaptchaToken) {
            setErrorMsg(t('admin.login.recaptchaRequired'));
            return;
        }
        setIsLoading(true);
        ...
        <div className="min-h-screen flex items-center justify-center login-bg-pattern relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md px-6">
                ...
                <div className="text-center mb-8 animate-fade-in-up">
                    ...
                    <p style={{ color: 'var(--color-text-muted)' }} className="">{t('admin.login.adminDashboard')}</p>
                </div>

                {/* Login Card */}
                <div className="glass-panel rounded-2xl p-8 shadow-custom-hover animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                    <h2 style={{ color: 'var(--color-text-heading)' }} className="font-heading text-2xl font-bold  mb-6">{t('admin.login.welcomeBack')}</h2>

                    <form onSubmit={handleSubmit} className="space-y-6">
                        {/* Email Field */}
                        <div>
                            <label style={{ color: 'var(--color-text-secondary)' }} className="block text-sm font-medium  mb-2">{t('admin.login.emailAddress')}</label>
                        ...
                        {/* Password Field */}
                        <div>
                            <label style={{ color: 'var(--color-text-secondary)' }} className="block text-sm font-medium  mb-2">{t('admin.login.password')}</label>
                            <div className="relative">
                                <input
                        ...
                        <div className="flex items-center justify-between">
                            <label className="flex items-center gap-2 cursor-pointer group">
                                ...
                                <span style={{ color: 'var(--color-text-muted)' }} style={{ color: 'var(--color-text-secondary)' }} className="text-sm  group-hover: transition-colors">{t('admin.login.rememberMe')}</span>
                            </label>
                            <a href="#" className="text-sm text-cyan-400 hover:text-cyan-300 transition-colors">{t('admin.login.forgotPassword')}</a>
                        </div>
                    ...
                    <div className="relative my-6">
                        ...
                        <div className="relative flex justify-center text-sm">
                            <span style={{ color: 'var(--color-text-muted)' }} className="px-4 bg-(--color-bg-secondary) ">{t('admin.login.orContinueWith')}</span>
                        </div>
                    </div>

                    {/* Social Buttons */}
                    <div className="grid grid-cols-2 gap-4">
                        <button type="button" onClick={() => window.location.assign(getOAuthUrl('google'))} className="btn-secondary py-3 flex items-center justify-center gap-2">
                            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12.545,10.239v3.821h5.445c-0.712,2.315-2.647,3.972-5.445,3.972c-3.332,0-6.033-2.701-6.033-6.032s2.701-6.032,6.033-6.032c1.498,0,2.866,0.549,3.921,1.453l2.814-2.814C17.503,2.988,15.139,2,12.545,2C7.021,2,2.543,6.477,2.543,12s4.478,10,10.002,10c8.396,0,10.249-7.85,9.426-11.748L12.545,10.239z" />
                            </svg>
                            <span className="text-sm font-medium">{t('admin.login.google')}</span>
                        </button>
                        <button type="button" onClick={() => window.location.assign(getOAuthUrl('github'))} className="btn-secondary py-3 flex items-center justify-center gap-2">
                            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <span className="text-sm font-medium">{t('admin.login.github')}</span>
                        </button>
                    </div>
                    {/* Footer */}
                    <div className="mt-8 pt-6 border-t  text-center">
                        <p className="text-sm text-gray-500">
                            {t('admin.login.secureAccess')} <span className="text-cyan-400 font-medium">{t('admin.login.algoArena')}</span>
                        </p>
                    </div>
                </div>
            </div>
