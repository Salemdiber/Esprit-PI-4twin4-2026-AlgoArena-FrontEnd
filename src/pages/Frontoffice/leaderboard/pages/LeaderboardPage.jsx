...
// Resolve the backend-stored avatar to a URL the browser can fetch.
// Mirrors the convention used in TopNavbar / Backoffice/Users.jsx:
//   - "http(s)://..."   → kept as-is (e.g. CDN, OAuth provider)
//   - "/uploads/foo.jpg"→ kept as-is (vite proxy forwards /uploads/* to backend)
//   - "uploads/foo.jpg" → prefixed with "/" so the proxy matches
//   - "data:..."        → kept as-is (already an inline asset)
//   - missing / falsy   → null  (caller should fall back to the initials SVG)
const getAvatarUrl = (user) => {
    ...
    // Unknown format → assume it's a filename under /uploads
...
const buildLeaderboardRow = (user, currentUser) => {
    ...
    return {
        ...
        username,
        // null / empty → AvatarMedia just shows DOM-rendered initials.
