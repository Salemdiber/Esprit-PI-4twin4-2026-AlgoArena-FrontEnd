    ...
    return (
        ...
        >
            ...
            <button
                ...
                onClick={onClose}
                aria-label={t('topNav.closeSidebar')}
            >
            ...
            <div className="p-6 flex justify-center" style={{ borderBottom: '1px solid var(--color-border)' }}>
                <img src={Logo} alt={t('sidebar.logoAlt')} className="h-8 w-auto" />
            </div>

            {/* Navigation */}
            <nav className="flex-1 p-4 overflow-y-auto custom-scrollbar">
                <SidebarLink to="/admin" label={t('sidebar.dashboard')} onClick={onClose}
                ...
                <SidebarLink to="/admin/users" label={t('sidebar.users')} onClick={onClose}
                ...
                <SidebarLink to="/admin/battles" label={t('sidebar.battles')} onClick={onClose}
                ...
                <SidebarLink to="/admin/leaderboards" label={t('sidebar.leaderboards')} onClick={onClose}
                ...
                <SidebarLink to="/admin/analytics" label={t('sidebar.analytics')} onClick={onClose}
                ...
                <SidebarLink to="/admin/community-analytics" label={t('sidebar.communityAnalytics')} onClick={onClose}
                    icon={<IconWrapper><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 8h10M7 12h6m-6 4h8M5 4h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V6a2 2 0 012-2z" /></IconWrapper>} />
                <SidebarLink to="/admin/system-health" label={t('sidebar.systemHealth')} onClick={onClose}
                ...
                <SidebarLink to="/admin/sessions" label={t('sidebar.sessions')} onClick={onClose}
                ...
                <SidebarLink to="/admin/activity-logs" label={t('sidebar.activityLogs')} onClick={onClose}
                ...
                <SidebarLink to="/admin/settings" label={t('sidebar.settings')} onClick={onClose}
            ...
            <div className="px-4 pb-3 flex justify-center border-b" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-lg glass-panel spotlight-hover cursor-pointer group">
                    ...
                    <div className="flex-1 relative z-10 overflow-hidden">
                        <p className="font-medium text-sm group-hover:text-cyan-400 transition-colors truncate" style={{ color: 'var(--color-text-primary)' }}>
                            {currentUser?.username || t('sidebar.adminUser')}
                        </p>
                        <p className="text-xs truncate" style={{ color: 'var(--color-text-muted)' }}>
                            {currentUser?.email || t('sidebar.systemAdmin')}
                        </p>
                    </div>
                </div>
            </div>
