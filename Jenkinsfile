pipeline {
  ...
  environment {
    ...
    SONAR_TEST_INCLUSIONS = 'src/**/__tests__/**/*.test.mjs,src/tests/**/*.test.mjs'
      ...
      steps {
        script {
          ...
          withSonarQubeEnv('SonarQube') {
            sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=${SONAR_PROJECT_KEY} -Dsonar.projectName=\"${SONAR_PROJECT_NAME}\" -Dsonar.sources=src -Dsonar.tests=src -Dsonar.test.inclusions=${SONAR_TEST_INCLUSIONS} -Dsonar.javascript.lcov.reportPaths=coverage/lcov.info"
          }
        }
      }
    }
