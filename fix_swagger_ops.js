...
function injectDetailedSwagger(file) {
    ...
    // Reverse the bad injection first
    code = code.replace(/    @ApiOperation\(\{[\s\S]*?@ApiResponse\(\{ status: 403, description: 'Forbidden - Insufficient permissions' \}\)\n/g, '');
    ...
    const newCode = code.replace(regex, (match, p1, p2) => {
        ...
        const methodName = verb + (route ? '_' + route.replace(/\W+/g, '_') : 'BaseRoute') + '_' + injectedMatch;
